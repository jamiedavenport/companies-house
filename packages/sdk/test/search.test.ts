...
import { describeLive, expectData } from "./fixtures.ts";

describeLive("search endpoints", () => {
  test("searchAll returns mixed results", async () => {
    const data = expectData(await searchAll({ query: { q: "tesco", items_per_page: 5 } }));
    expect(data.items?.length).toBeGreaterThan(0);
  });

  test("searchCompanies finds companies", async () => {
    const data = expectData(await searchCompanies({ query: { q: "tesco", items_per_page: 5 } }));
    expect(data.items?.length).toBeGreaterThan(0);
    expect(data.items?.[0]?.company_number).toBeTypeOf("string");
  });

  test("searchOfficers finds officers", async () => {
    const data = expectData(await searchOfficers({ query: { q: "smith", items_per_page: 5 } }));
    expect(data.items?.length).toBeGreaterThan(0);
  });

  test("searchDisqualifiedOfficers finds officers", async () => {
    const data = expectData(
      await searchDisqualifiedOfficers({ query: { q: "smith", items_per_page: 5 } }),
    );
    expect(data.items?.length).toBeGreaterThan(0);
  });
  ...
  test("searchDissolvedCompanies finds companies", async () => {
    ...
    const data = expectData(
      await searchDissolvedCompanies({ query: { q: "woolworths", search_type: "alphabetical" } }),
    );
    expect(data.items?.length).toBeGreaterThan(0);
  });

  test("searchCompaniesAlphabetically finds companies", async () => {
    const data = expectData(await searchCompaniesAlphabetically({ query: { q: "tesco" } }));
    expect(data.items?.length).toBeGreaterThan(0);
  });

  test("advancedCompanySearch filters companies", async () => {
    const data = expectData(
      await advancedCompanySearch({

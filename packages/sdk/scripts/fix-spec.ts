...
type Tag = { name: string; description?: string; "x-displayName"?: string };
...
function fixSecurity(spec: Spec): void {
...
function fixPathTemplates(spec: Spec): void {
...
function stripLeakedPathItemKeys(spec: Spec): void {
...
function fixLegacyParameters(spec: Spec): void {
  for (const [, , op] of allOperations(spec)) {
...
function assignOperationIds(spec: Spec): number {
...
function hoistObjectItems(root: unknown): number {
  let hoisted = 0;
...
function fixChargeLinks(spec: Spec): void {
...
function relaxPaginationParams(spec: Spec): void {
  for (const [, , op] of allOperations(spec)) {
    for (const param of op.parameters ?? []) {
      if (
        param.in === "query" &&
...
function normalizeTags(spec: Spec): void {
